...
// content : Long Arithmetic, Arithmetic Operators with Assignment,
//           Binary Search Algorithm, Postfix and Prefix Operators,
//           Arithmetic and Comparison Operators, Operators >> and <<,
//           Square Roots, Karatsuba Fast Multiplication Algorithm,
//           Functions std::ssize, std::isdigit, std::stoll, Radix Optimization
// additions: remainder (%), exponentiation (pow), sign, absolute value
...
class Integer {
  public:
    ...
    friend auto operator==(Integer const &lhs, Integer const &rhs) -> bool {
        ...
        return std::equal(lhs.m_digits.begin(), lhs.m_digits.end(), rhs.m_digits.begin(),
                          rhs.m_digits.end());
    }
    ...
    friend auto multiply(Integer const &x, Integer const &y) -> Integer {
        if (auto size = std::max(x.m_size, y.m_size); size > 1) {
            ...
            for (std::size_t i = step; i < size; ++i) {
  ...
  private:
    ...
    Integer &add(Integer const &other) {
        ...
        if (m_digits[m_size]) { // carry into a new digit
            ++m_size;
        }
    ...
    static inline std::size_t s_size = 1'000;
